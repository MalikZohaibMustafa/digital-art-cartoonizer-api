...
from flask import Flask, flash, render_template, request, send_file, jsonify
...
def submit():
    ...
    print(request.files['content_img'])
    print(request.files["style_img"])
    ...
    #  while selecting image
        ...
        art_img = Image.fromarray((art_img * 255).astype(np.uint8))        
        art_img.save(generated_art_path)
    
        return jsonify({'image':'http://localhost:7000/'+generated_art_path})
...
def share_art():
    ...
    """        
    return render_template('share_art.html', art_file = generated_art_name)
